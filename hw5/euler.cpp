...
bool Eulerian(std::vector<std::vector<int>> theGraph, std::vector<int>& circuit){
	...
	if (odds > 2){
		...
		while (circuit.size() <= tedges){
			...
			if (current == trav and !took.empty()){
				circuit.push_back(took.top());
				current = took.top();
				took.pop();
			}
...
int main (int argc, char** argv){
	...
	if (Eulerian(Graph,taken)){
		...
		if (taken[taken.size()-1]==0){
				std::cout<<"a ";
			}
			if (taken[taken.size()-1]==1){
				std::cout<<"b ";
			}
			if (taken[taken.size()-1]==2){
				std::cout<<"c ";
			}
			if (taken[taken.size()-1]==3){
				std::cout<<"d ";
			}
			if (taken[taken.size()-1]==4){
				std::cout<<"e ";
			}
			if (taken[taken.size()-1]==5){
				std::cout<<"f ";
			}
			if (taken[taken.size()-1]==6){
				std::cout<<"g ";
			}
