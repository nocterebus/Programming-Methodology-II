...
(make a left and right half copy vector and write back)
...
Here the number of moves is the number of placements from the left and right arrays and comparisons is number of comparisons (both should be tightly bound)
*/ 

//Include header file that contains file reader and writer
#include "initial.h"

//sorter and merger for mergesort that uses a copy array
void merge(std::vector<int>::iterator begin, std::vector<int>::iterator middle, std::vector<int>::iterator end, long int& compares, long int& moves){
	...
	for (std::vector<int>::iterator copy = sorted.begin(); copy != sorted.end(); ++copy){
		...
		if (left < middle and *left <= *right){
			*copy = *left;
			//increment left iterator and compares
			++left;
            ++compares;
		}
		//if right element is less than left element, place right element
			...
			*copy = *right;
			//increment right iterator and compares
			++right;
            ++compares;
		}
